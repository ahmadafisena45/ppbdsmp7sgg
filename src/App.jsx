 ...
 * Pastikan data di bawah ini sudah diganti dengan data asli dari Firebase Console Anda.
 */
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        setError("Koneksi Firebase Gagal. Periksa API Key dan Aktifkan 'Anonymous Sign-in' di Firebase Auth.");
      }
    };
    initAuth();
  ...
  useEffect(() => {
    if (isAdmin && user) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        setError("Firestore Error: Pastikan Rules di Firebase sudah diset ke 'allow read, write: if true;'");
      });
      return () => unsubscribe();
    }
  }, [isAdmin, user]);
  ...
  const submitRegistration = async (e) => {
    e.preventDefault();
    if (!user) {
      setError("Sistem belum siap. Silakan refresh halaman.");
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      if (GAS_URL) {
        ...
        }).catch(err => console.log("GAS Sync Error"));
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-red-50 p-6 text-center">
        <div className="max-w-md space-y-4">
          ...
          <h1 className="text-2xl font-bold text-red-800">Konfigurasi Belum Lengkap</h1>
          <p className="text-red-600">Anda belum memasukkan <b>API Key</b>, <b>App ID</b>, dan <b>Sender ID</b> yang asli dari Firebase Console ke dalam file App.jsx.</p>
          <p className="text-sm text-slate-500">Silakan edit bagian <code className="bg-white p-1">const firebaseConfig</code> di kode Anda lalu deploy ulang.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <header className="bg-blue-700 text-white p-4 flex justify-between items-center shadow-lg sticky top-0 z-50 no-print">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setCurrentView('home')}>
          <ShieldCheck size={24} />
          <h1 className="font-bold tracking-tight uppercase">SMPN 7 SINGINGI</h1>
        </div>
      ...
      <main className="p-4 max-w-lg mx-auto pt-8">
        {error && (
          <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6 rounded-r-xl animate-bounce text-sm">
            {error}
          </div>
        )}
        ...
        {currentView === 'home' && (
          <div className="text-center space-y-8 py-10 animate-in fade-in duration-500">
            ...
            <div className="space-y-3">
              <button onClick={() => setCurrentView('register')} className="w-full bg-blue-600 text-white p-4 rounded-2xl font-bold shadow-xl hover:bg-blue-700 transition-all active:scale-95">Daftar Sekarang</button>
              <button onClick={() => setCurrentView('login')} className="w-full bg-white border-2 border-blue-100 text-blue-600 p-4 rounded-2xl font-bold hover:bg-blue-50 transition-all active:scale-95">Cek Status Siswa</button>
            </div>
          </div>
        )}
        ...
        {currentView === 'register' && (
          <form onSubmit={submitRegistration} className="bg-white p-6 rounded-3xl shadow-xl space-y-4 border border-slate-100">
            ...
            <button disabled={loading} className="w-full bg-blue-600 text-white p-4 rounded-2xl font-bold shadow-lg mt-4">{loading ? "Mengirim..." : "Kirim Pendaftaran"}</button>
        ...
        {currentView === 'admin-login' && (
          <form onSubmit={(e) => { e.preventDefault(); if(adminLogin.password === 'smpn7singingi') {setIsAdmin(true); setCurrentView('admin-dashboard');} else {setError("Sandi Admin Salah!")}} } className="bg-white p-8 rounded-3xl shadow-xl space-y-6">
            <h2 className="text-xl font-black text-slate-800 text-center uppercase tracking-tight">Otoritas</h2>
            <input required placeholder="Sandi Admin" type="password" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-center font-bold" onChange={(e) => setAdminLogin({...adminLogin, password: e.target.value})} />
            <button className="w-full bg-slate-900 text-white p-4 rounded-2xl font-bold">Buka Panel</button>
          </form>
        )}
        ...
        {currentView === 'admin-dashboard' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => {setIsAdmin(false); setCurrentView('home')}} className="text-red-500"><LogOut size={24}/></button>
            </div>
            <div className="space-y-3">
              {allApplicants.map(applicant => (
                <div key={applicant.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center group">
                  <div className="overflow-hidden">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{applicant.status}</p>
                  </div>
